...
voxel2clip_kwargs = dict(in_dim=num_voxels,out_dim=out_dim, dropout_rate=dropout_rate, projector_dropout=projector_dropout)
...
for mcd_forward_passes in [1,5,10,25]:
    ...
    for mode in ["imagery", "vision"]:
        ...
        blurry_stats = torch.zeros((voxels.shape[0], 3))
        clip_stats = torch.zeros((voxels.shape[0], 3))
        stat_path = f"/export/raid1/home/kneel027/Second-Sight/output/mental_imagery_paper_b3/{mode}/mindeye_mcd_{mcd_forward_passes}passes_{dropout_rate}dr/subject{subj}/"
        ...
        for val_i, (voxel, img) in enumerate(tqdm(zip(voxels.to(device), images.to(device)))):
            ...
            with torch.no_grad():
                if img2img:
                    ae_outputs, blurry_mcd_variance, blurry_mcd_entropy, blurry_mcd_mutual_info = utils.monte_carlo_inference(data=voxel.float(),
                                                                                                                        forward_passes=mcd_forward_passes, 
                                                                                                                        model=voxel2sd,
                                                                                                                        use_projector=False)
                    ae_preds = torch.mean(ae_outputs[0],axis=0).reshape((1,4,64,64))
                    # print(ae_preds.shape, blurry_mcd_variance.shape, blurry_mcd_entropy.shape, blurry_mcd_mutual_info.shape)
                    blurry_recons = vd_pipe.vae.decode(ae_preds.to(device).half()/0.18215).sample / 2 + 0.5
                    blurry_stats[val_i,0] = blurry_mcd_variance
                    blurry_stats[val_i,1] = blurry_mcd_entropy
                    blurry_stats[val_i,2] = blurry_mcd_mutual_info
                    if val_i==0:
                ...
                if only_lowlevel:
                    ...
                    grid, brain_recons, laion_best_picks, recon_img, extracted_clips, clip_mcd_variance, clip_mcd_entropy, clip_mcd_mutual_info = utils.reconstruction_mcd(
                        img, voxel,
                    ...
                    clip_stats[val_i, 0] = clip_mcd_variance
                    clip_stats[val_i, 1] = clip_mcd_entropy
                    clip_stats[val_i, 2] = clip_mcd_mutual_info
                    if plotting:
                        plt.show()
                    ...
                    rec_path = f"/export/raid1/home/kneel027/Second-Sight/output/mental_imagery_paper_b3/{mode}/mindeye_mcd_{mcd_forward_passes}passes_{dropout_rate}dr/subject{subj}/{val_i}/"
                    ...
                    # Retrieval CLIP
                    outputs, retrieval_mcd_variance, retrieval_mcd_entropy, retrieval_mcd_mutual_info = utils.monte_carlo_inference(data=voxel.float(),
                                                                                                                        forward_passes=mcd_forward_passes, 
                                                                                                                        model=diffusion_prior.voxel2clip,
                                                                                                                        use_projector=True)
                    extracted_clip_hidden = torch.mean(outputs[1], axis=0)
                    torch.save(extracted_clip_hidden, f"{rec_path}contrastive_clip_hidden_{rep}.pt")
                    torch.save(outputs[1], f"{rec_path}contrastive_clip_hidden_block_{rep}.pt")
        ...
        #     break
        torch.save(clip_stats, f"{stat_path}clip_stats_{rep}.pt")
        torch.save(blurry_stats, f"{stat_path}blurry_stats_{rep}.pt")
